// src/api/endpoints/material/pant.material.endpoints.js
import { pantMaterialApiSlice } from "../../slices";

export const pantMaterialApi = pantMaterialApiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getPantMaterials: builder.query({
    ...
    updatePantMaterial: builder.mutation({
      query: ({ id, data }) => ({
        url: `/${id}`,
        method: "PUT",
        credentials: "include",
      ...
      onQueryStarted: async (
        ...
        const patchResult = dispatch(
          pantMaterialApi.util.updateQueryData(
            "getPantMaterials",
            { skip, limit: 6 },
            (draft) => {
    ...
    deletePantMaterial: builder.mutation({
      query: ({ id }) => ({
        url: `/${id}`,
        method: "DELETE",
        credentials: "include",
    ...
    createPantMaterial: builder.mutation({
      query: ({ data }) => {
