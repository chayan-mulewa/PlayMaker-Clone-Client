// src/api/endpoints/material/jeans.material.endpoints.js
import { jeansMaterialApiSlice } from "../../slices";

export const jeansMaterialApi = jeansMaterialApiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getJeansMaterials: builder.query({
    ...
    updateJeansMaterial: builder.mutation({
      query: ({ id, data }) => ({
        url: `/${id}`,
        method: "PUT",
        credentials: "include",
      ...
      onQueryStarted: async (
        ...
        const patchResult = dispatch(
          jeansMaterialApi.util.updateQueryData(
            "getJeansMaterials",
            { skip, limit: 6 },
            (draft) => {
    ...
    deleteJeansMaterial: builder.mutation({
      query: ({ id }) => ({
        url: `/${id}`,
        method: "DELETE",
        credentials: "include",
    ...
    createJeansMaterial: builder.mutation({
      query: ({ data }) => {
