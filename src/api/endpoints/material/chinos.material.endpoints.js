// src/api/endpoints/material/chinos.material.endpoints.js
import { chinosMaterialApiSlice } from "../../slices";

export const chinosMaterialApi = chinosMaterialApiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getChinosMaterials: builder.query({
    ...
    updateChinosMaterial: builder.mutation({
      query: ({ id, data }) => ({
        url: `/${id}`,
        method: "PUT",
        credentials: "include",
      ...
      onQueryStarted: async (
        ...
        const patchResult = dispatch(
          chinosMaterialApi.util.updateQueryData(
            "getChinosMaterials",
            { skip, limit: 6 },
            (draft) => {
    ...
    deleteChinosMaterial: builder.mutation({
      query: ({ id }) => ({
        url: `/${id}`,
        method: "DELETE",
        credentials: "include",
    ...
    createChinosMaterial: builder.mutation({
      query: ({ data }) => {
