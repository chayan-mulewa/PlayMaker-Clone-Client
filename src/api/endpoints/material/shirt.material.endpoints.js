// src/api/endpoints/material/shirt.material.endpoints.js
import { shirtMaterialApiSlice } from "../../slices";
import { nanoid } from "nanoid";

export const shirtMaterialApi = shirtMaterialApiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getShirtMaterials: builder.query({
    ...
    updateShirtMaterial: builder.mutation({
      query: ({ id, data }) => ({
        url: `/${id}`,
        method: "PUT",
        credentials: "include",
      ...
      onQueryStarted: async (
        ...
        const patchResult = dispatch(
          shirtMaterialApi.util.updateQueryData(
            "getShirtMaterials",
            { skip, limit: 6 },
            (draft) => {
    ...
    deleteShirtMaterial: builder.mutation({
      query: ({ id }) => ({
        url: `/${id}`,
        method: "DELETE",
        credentials: "include",
    ...
    createShirtMaterial: builder.mutation({
      query: ({ data }) => {
