...
import { coatMaterialApi, cartApi } from "../api/endpoints";
...
export default function Coats() {
  ...
  const { data, isLoading } = coatMaterialApi.useGetCoatMaterialsQuery({
    skip,
    limit,
  });
  ...
  const measurementType = useMemo(
    () => [
      ...
      {
        label: "Coat Length (cm)",
        name: "coatLength",
        rules: [{ required: true, message: "Coat Length length is required" }],
        min: 60,
        max: 120,
        initialValue: 75,
      },
  ...
  const handleMeasurementSubmit = (values) => {
    ...
    messageApi.open({
      type: "success",
      content: "Measurements For Coat Is Created",
    });
  };
  ...
  const handleAddToCart = async () => {
    const data = {
      selectedMaterial: { type: "CoatMaterial", ...selectedMaterial },
      selectedDesign: { type: "CoatDesign", ...selectedDesigns },
      selectedMeasurement: {
        type: "CoatMeasurement",
