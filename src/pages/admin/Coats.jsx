...
import { coatMaterialApi } from "../../api/endpoints";
...
export default function Coats() {
  ...
  const [createCoatMaterial] =
    coatMaterialApi.useCreateCoatMaterialMutation();
  const [updateCoatMaterial] =
    coatMaterialApi.useUpdateCoatMaterialMutation();
  const [deleteCoatMaterial] =
    coatMaterialApi.useDeleteCoatMaterialMutation();
  ...
  const handleCreateMaterial = async (values) => {
    ...
    try {
      const res = await createCoatMaterial({
        data: newMaterial,
        skip,
      }).unwrap();
  ...
  const handleUpdateMaterial = async (values) => {
    ...
    try {
      const res = await updateCoatMaterial({
  ...
  const handleDeleteMaterial = async (id) => {
    try {
      const res = await deleteCoatMaterial({ id, skip }).unwrap();
      messageApi.success(res.message);
    } catch (error) {
