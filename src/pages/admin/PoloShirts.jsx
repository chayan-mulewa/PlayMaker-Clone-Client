...
import { poloShirtMaterialApi } from "../../api/endpoints";
...
export default function PoloShirts() {
  ...
  // API Calls
  const { data, isLoading } = poloShirtMaterialApi.useGetPoloShirtMaterialsQuery({
    skip,
    limit,
  });
  const [createPoloShirtMaterial] =
    poloShirtMaterialApi.useCreatePoloShirtMaterialMutation();
  const [updatePoloShirtMaterial] =
    poloShirtMaterialApi.useUpdatePoloShirtMaterialMutation();
  const [deletePoloShirtMaterial] =
    poloShirtMaterialApi.useDeletePoloShirtMaterialMutation();
  ...
  const handleCreateMaterial = async (values) => {
    ...
    try {
      const res = await createPoloShirtMaterial({
        data: newMaterial,
        skip,
      }).unwrap();
  ...
  const handleUpdateMaterial = async (values) => {
    ...
    try {
      const res = await updatePoloShirtMaterial({
  ...
  const handleDeleteMaterial = async (id) => {
    try {
      const res = await deletePoloShirtMaterial({ id, skip }).unwrap();
      messageApi.success(res.message);
    } catch (error) {
