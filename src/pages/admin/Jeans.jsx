...
import { jeansMaterialApi } from "../../api/endpoints";
...
export default function Jeans() {
  ...
  // API Calls
  const { data, isLoading } = jeansMaterialApi.useGetJeansMaterialsQuery({
    skip,
    limit,
  });
  const [createJeansMaterial] =
    jeansMaterialApi.useCreateJeansMaterialMutation();
  const [updateJeansMaterial] =
    jeansMaterialApi.useUpdateJeansMaterialMutation();
  const [deleteJeansMaterial] =
    jeansMaterialApi.useDeleteJeansMaterialMutation();
  ...
  const handleCreateMaterial = async (values) => {
    ...
    try {
      const res = await createJeansMaterial({
        data: newMaterial,
        skip,
      }).unwrap();
  ...
  const handleUpdateMaterial = async (values) => {
    ...
    try {
      const res = await updateJeansMaterial({
  ...
  const handleDeleteMaterial = async (id) => {
    try {
      const res = await deleteJeansMaterial({ id, skip }).unwrap();
      messageApi.success(res.message);
    } catch (error) {
