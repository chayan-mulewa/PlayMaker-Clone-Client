...
import { pantMaterialApi } from "../../api/endpoints";
...
export default function Pants() {
  ...
  // API Calls
  const { data, isLoading } = pantMaterialApi.useGetPantMaterialsQuery({
    skip,
    limit,
  });
  const [createPantMaterial] =
    pantMaterialApi.useCreatePantMaterialMutation();
  const [updatePantMaterial] =
    pantMaterialApi.useUpdatePantMaterialMutation();
  const [deletePantMaterial] =
    pantMaterialApi.useDeletePantMaterialMutation();
  ...
  const handleCreateMaterial = async (values) => {
    ...
    try {
      const res = await createPantMaterial({
        data: newMaterial,
        skip,
      }).unwrap();
  ...
  const handleUpdateMaterial = async (values) => {
    ...
    try {
      const res = await updatePantMaterial({
  ...
  const handleDeleteMaterial = async (id) => {
    try {
      const res = await deletePantMaterial({ id, skip }).unwrap();
      messageApi.success(res.message);
    } catch (error) {
