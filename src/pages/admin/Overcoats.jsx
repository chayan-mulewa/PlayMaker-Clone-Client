...
import { overcoatMaterialApi } from "../../api/endpoints";
...
export default function Overcoats() {
  ...
  const [createOvercoatMaterial] =
    overcoatMaterialApi.useCreateOvercoatMaterialMutation();
  const [updateOvercoatMaterial] =
    overcoatMaterialApi.useUpdateOvercoatMaterialMutation();
  const [deleteOvercoatMaterial] =
    overcoatMaterialApi.useDeleteOvercoatMaterialMutation();
  ...
  const handleCreateMaterial = async (values) => {
    ...
    try {
      const res = await createOvercoatMaterial({
        data: newMaterial,
        skip,
      }).unwrap();
  ...
  const handleUpdateMaterial = async (values) => {
    ...
    try {
      const res = await updateOvercoatMaterial({
  ...
  const handleDeleteMaterial = async (id) => {
    try {
      const res = await deleteOvercoatMaterial({ id, skip }).unwrap();
      messageApi.success(res.message);
    } catch (error) {
