...
import { overcoatMaterialApi, cartApi } from "../api/endpoints";
...
export default function Overcoats() {
  ...
  const { data, isLoading } = overcoatMaterialApi.useGetOvercoatMaterialsQuery({
    skip,
    limit,
  });
  ...
  const measurementType = useMemo(
    () => [
      ...
      },
      {
        label: "Wrist (cm)",
        name: "wrist",
        rules: [{ required: true, message: "Wrist size is required" }],
        min: 10,
        max: 30,
        initialValue: 10,
      },
      {
        label: "Elbow (cm)",
        name: "elbow",
        rules: [{ required: true, message: "Elbow size is required" }],
        min: 10,
        max: 30,
        initialValue: 10,
      },
      {
        label: "Lapel Width (cm)",
        name: "lapelWidth",
        rules: [{ required: true, message: "Lapel Width size is required" }],
        min: 5,
        max: 15,
        initialValue: 10,
      },
      ...
      {
        label: "Overcoat Length (cm)",
        name: "overcoatLength",
        rules: [
          { required: true, message: "Overcoat Length length is required" },
        ],
        min: 60,
        max: 120,
        initialValue: 75,
  ...
  const handleMeasurementSubmit = (values) => {
    ...
    messageApi.open({
      type: "success",
      content: "Measurements For Overcoat Is Created",
    });
  };
  ...
  const handleAddToCart = async () => {
    const data = {
      selectedMaterial: { type: "OvercoatMaterial", ...selectedMaterial },
      selectedDesign: { type: "OvercoatDesign", ...selectedDesigns },
      selectedMeasurement: {
        type: "OvercoatMeasurement",
